...
const DislikeNotes=({item, handleClick,handleChange, noteForm, setNoteForm,refresh,setRefresh})=>{
    ...
    //toggles between whether the Note page is visible or not
    const handleNote=()=>{
    ...
    //toggles between whether the message is on the editable page or the read-only page
    ...
    //when the item is saved, message will be posted to the DB
    //there is an if/else to distinguish whether we are updating item in the favorite or dislike bin 
    const SaveNote=(_id)=>{
        if(toggle==true){
            setToggle(false);
        }
